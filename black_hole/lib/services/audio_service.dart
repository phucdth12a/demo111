...
import 'package:black_hole/services/yt_music.dart';
...
class AudioPlayerHandlerImpl extends BaseAudioHandler
  ...
  Stream<List<IndexedAudioSource>> get _effectiveSequence => Rx.combineLatest3<
          ...
          (sequence, shuffleIndices, shuffleModeEnable) {
        if (sequence == null) return [];
        if (!shuffleModeEnable) return sequence;
  ...
  int? getQueueIndex(int? currentIndex, List<int>? shuffleIndices,
      {bool shuffleModeEnable = false}) {
    final effectiveIndices = _player!.effectiveIndices ?? [];
    final shuffleIndicesInv = List.filled(effectiveIndices.length, 0);
    for (var i = 0; i < effectiveIndices.length; i++) {
      shuffleIndicesInv[effectiveIndices[i]] = i;
    }
    return (shuffleModeEnable &&
  ...
  Future<void> _init() async {
    ...
    preferredQuality =
        Hive.box('settings').get('streamingQuality', defaultValue: '96 kpps');
    resetOnSkip = Hive.box('settings').get('resetOnSkip', defaultValue: false);
    cacheSong = Hive.box('settings').get('cacheSong', defaultValue: true);
      ...
      if (item.artUri.toString().startsWith('http')) {
        ...
        if (recommend && item.extras!['autoplay'] as bool) {
          ...
          if (queueLength - index < 5) {
            Logger.root.info('less than 5 sóng remaining, adding more songs');
            Future.delayed(const Duration(seconds: 1), () async {
              if (item == mediaItem.value) {
                if (item.genre != 'Youtube') {
                  final value = await SaavnAPI().getReco(item.id);
                  value.shuffle();

                  for (int i = 0; i < value.length; i++) {
                    final element = MediaItemConverter.mapToMediaItem(value[i],
                        addedByAutoplay: true);
                    if (!mediaQueue.map((e) => e.id).contains(element.id)) {
    ...
    Rx.combineLatest4<int?, List<MediaItem>, bool, List<int>?, MediaItem?>(
        ...
        (index, queue, shuffleModeEnable, shuffleIndices) {
      final queueIndex = getQueueIndex(
        index,
        shuffleIndices,
        shuffleModeEnable: shuffleModeEnable,
      );
    ...
    try {
      if (loadStart) {
        final String lastQueueListData =
            await Hive.box('cache').get('lastQueue', defaultValue: '');
        List<MusicModel> lastQueueList = [];
        if (lastQueueListData.isNotEmpty) {
          final lastQueueListJson =
              jsonDecode(lastQueueListData) as List<dynamic>;
          lastQueueList = lastQueueListJson
              .map((e) => MusicModel.fromJson(e as Map<String, dynamic>))
              .toList();
        }

        final int lastIndex =
            await Hive.box('cache').get('lastIndex', defaultValue: 0);

        final int lastPos =
            await Hive.box('cache').get('lastPos', defaultValue: 0);

        if (lastQueueList.isNotEmpty &&
            lastQueueList.first.genre != 'YouTube') {
          final List<MediaItem> lastQueue = lastQueueList
              .map((e) => MediaItemConverter.mapToMediaItem(e))
              .toList();
  ...
  Future<void> refreshLink(MusicModel newData) async {
  ...
  Future customAction(String name, [Map<String, dynamic>? extras]) {
    ...
    if (name == 'refreshLink') {
      if (extras?['newData'] != null) {
        refreshLink(
            MusicModel.fromJson(extras!['newData'] as Map<String, dynamic>));
      }
    }
  ...
  void _broadcastState(PlaybackEvent event) {
    ...
    final queueIndex = getQueueIndex(
      ...
      shuffleModeEnable: _player!.shuffleModeEnabled,
    );
    playbackState.add(
