...
class MyTheme with ChangeNotifier {
  bool _isDark = Hive.box('settings').get('darkMode', defaultValue: true);

  bool _useSystemTheme =
      Hive.box('settings').get('useSystemThem', defaultValue: false);
  ...
  String cardColor =
      Hive.box('settings').get('cardColor', defaultValue: 'Grey900');

  int backGrad = Hive.box('settings').get('getGrad', defaultValue: 2);
  int cardGrad = Hive.box('settings').get('cardGrad', defaultValue: 4);
  int bottomGrad = Hive.box('settings').get('bottomGrad', defaultValue: 3);
  ...
  void refresh() {
    ...
    _isDark = settingsBox.get('darkMode', defaultValue: true);

    _useSystemTheme = settingsBox.get('useSystemTheme', defaultValue: false);

    accentColor = settingsBox.get('themeColor', defaultValue: 'Teal');
    canvasColor = settingsBox.get('canvasColor', defaultValue: 'Grey');
    cardColor = settingsBox.get('cardColor', defaultValue: 'Grey900');

    backGrad = settingsBox.get('backGrad', defaultValue: 2);
    cardGrad = settingsBox.get('cardGrad', defaultValue: 4);
    bottomGrad = settingsBox.get('bottomGrad', defaultValue: 3);

    colorHue = settingsBox.get('colorHue', defaultValue: 400);
    notifyListeners();
  }
  ...
  Map getThemes() {
    return Hive.box('settings').get('userthemes', defaultValue: {});
  }
  ...
  String getInitialTheme() {
    return Hive.box('settings').get('theme');
  }
}
